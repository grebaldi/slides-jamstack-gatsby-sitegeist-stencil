...
class TemplateWrapper extends Component {
	...
	render() {
		const { location, children, site, logo } = this.props;
		...
		return (
			<div>
				...
				<footer>
					<img src={logo} alt="Sitegeist Logo" />
				</footer>
			</div>
		);
	}
	...
	<StaticQuery
		query={graphql`
			query IndexQuery {
				...
				allFile(filter: {relativePath: {eq: "logo-sitegeist-bright.png"}}) {
					edges {
						node {
							publicURL
						}
		...
		render={data => (
			<TemplateWrapper
				site={data.site}
				logo={data.allFile.edges[0].node.publicURL}
